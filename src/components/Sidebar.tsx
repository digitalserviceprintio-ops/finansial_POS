...
export const Sidebar: React.FC = () => {
  ...
  const handleMouseLeave = () => {
    ...
    // Smooth delay before auto-closing when cursor leaves
  ...
  const handleNavClick = (tab: MainTab) => {
    ...
    if (typeof window !== 'undefined' && window.innerWidth < 1024) {
      ...
      // Auto-close when item is clicked in hover-auto mode
      setIsHovered(false);
    }
  };
  ...
  return (
    <>
      {/* Mobile Dark Backdrop with smooth fade transition */}
      <div
        className={`fixed inset-0 z-40 bg-slate-950/60 backdrop-blur-xs transition-opacity duration-300 ease-in-out lg:hidden ${
      ...
      {/* Desktop Left-Edge Sensor for Auto Open on Hover */}
      {!effectiveOpen && (
        <div
          onMouseEnter={handleMouseEnter}
          className="hidden lg:flex fixed left-0 top-16 bottom-0 w-3 hover:w-5 z-40 cursor-pointer items-center justify-center group transition-all duration-200"
          title="Arahkan kursor ke sini untuk membuka menu otomatis (Hover)"
        >
          <div className="h-16 w-1 rounded-r-full bg-[#4648d4]/30 group-hover:bg-[#4648d4] group-hover:w-1.5 group-hover:h-24 transition-all duration-300 shadow-xs" />
        </div>
      )}
      ...
      <aside
        ...
        className={`fixed inset-y-0 left-0 z-50 flex flex-col justify-between border-r bg-white transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] no-scrollbar lg:static lg:z-auto lg:h-[calc(100vh-4rem)] ${
          effectiveOpen
            ? 'w-72 max-w-[85vw] translate-x-0 shadow-2xl border-[#e2e1ec] lg:w-64 lg:translate-x-0 lg:opacity-100 lg:shadow-none'
            : '-translate-x-full pointer-events-none border-transparent lg:w-0 lg:translate-x-0 lg:opacity-0 lg:border-r-0 lg:overflow-hidden lg:pointer-events-none'
        }`}
      >
        <div className="w-72 max-w-[85vw] lg:w-64 shrink-0 flex flex-col justify-between h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-5 no-scrollbar">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-[#4648d4] to-[#3435ad] p-3.5 text-white shadow-md">
              ...
              <button
                ...
                className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-white py-2 text-xs font-bold text-[#4648d4] shadow-xs hover:bg-[#f3f2fa] transition-all cursor-pointer"
              >
            ...
            <div>
              <div className="flex items-center justify-between px-2.5">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#767680]">
                  Menu Utama
                </span>
                <button
                  type="button"
                  onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                  className="hidden lg:flex items-center gap-1.5 rounded-lg px-2 py-0.5 text-[10px] font-bold transition-all cursor-pointer text-[#4648d4] hover:bg-[#ebeaff] border border-[#ebeaff]"
                  title={
                    isSidebarOpen
                      ? 'Menu disematkan (Klik untuk aktifkan mode buka-tutup otomatis)'
                      : 'Mode buka-tutup otomatis aktif (Klik untuk sematkan menu)'
                  }
                >
                  {isSidebarOpen ? (
                    <>
              ...
              {bottomNavItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => handleNavClick(item.id)}
                    className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
                      isActive
